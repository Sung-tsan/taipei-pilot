...
- [x] 追焦相機起降「對正」再驗（v5.1-2 HITL 第二輪：真因＝追焦方向落後→轉彎四分之三斜視；已把位置追 3.2→6、注視點 5→11、移除 bank 傾斜＝緊跟正後方+地平線水平）。若仍偏＝再查 GLB bbox 中心是否非機身中心
  → 2026-07-11 證實＝GLB bbox 偏心 × yaw=π 疊加：fitToLength 置中在未旋轉座標算，yaw 套在位移之外讓偏心翻倍甩出（a330 橫偏 ~10m＝鏡頭像在右後方、B737 縱偏 ~19m＝鏡頭貼機尾下降穿模）。已修：plane-entity._buildGlb 旋轉後重量 bbox 拉回置中；另加 spec 級 cam back/up 倍率（B737 1.15/1.35、A330 1.1/1.3，下降時機尾不頂鏡頭，數值待真機校）
  → **P0-1 2026-09-07 HITL 回歸**：仍感斜後方。複驗 GLB 置中／yaw 仍正確；真因＝chase-cam 世界座標 lerp 在轉彎時抄近路離開 −heading 軸（穩態橫偏）。已改 XZ 幾何鎖正後方（只平滑 Y）；GLB 置中改為 yaw 後一律重算；unit test 斷言橫向分量≈0。
- [x] B737/A330 下降視角（cam 倍率）真機再校：俯衝時鏡頭是否仍被高尾翼頂到／視角會不會太高（plane-specs.js model.cam）— HITL 2026-07-12 Sung pass（B737 1.15/1.35、A330 1.1/1.3 定案）
