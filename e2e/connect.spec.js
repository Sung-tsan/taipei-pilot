...
// —— RWD：小手機橫拿（iPhone 12 等）操控面板曾顯示不全（底部喇叭/降落鍵被裁）。HITL 2026-06-13 Sung 回報。——
// Chromium 無 iOS 動態工具列，無法重現 inset:0 用大 viewport 的落差；故用「刻意偏矮」的橫螢幕高度當判別器：
// 舊版（大泡泡 + 固定 min-height + 1fr 不可縮）在此高會溢出裁切，新版（dvh + minmax(0,1fr) + 短螢幕 media query 縮放）剛好放得下。
// 已實證：還原 remote.html 的 RWD 修正會讓本測試轉紅。
const RWD_CASES = /** @type {const} */ ([
  { scheme: 'simple', q: '', h: 250 },
  { scheme: 'complex', q: '&scheme=complex', h: 330 }, // 複雜版多一列儀表/舵/襟翼，需高一點
]);
for (const c of RWD_CASES) {
  test(`小手機橫拿短螢幕（${c.scheme}, ${c.h}px）→ 操控面板完整顯示不裁切（RWD regression）`, async ({ browser }) => {
    const ctx = await browser.newContext({
      ignoreHTTPSErrors: true,
      viewport: { width: 812, height: c.h },
    });
    const page = await ctx.newPage();
    await page.goto(`/remote.html?test=1${c.q}`);
    await page.click('#startBtn');
    await page.click('#calDoneBtn');
    await expect(page.locator('#control')).toBeVisible();

    const ids = ['slotBanner', 'leftCol', 'midCol', 'thrTrack', 'ctxKeys', 'gearBtn', 'calBtn', 'attitude'];
    if (c.scheme === 'complex') ids.push('complexBar', 'instruments', 'rudderTrack');

    // 任一操控元件越界（被視窗裁切）或塌成 0 → 視為失敗
    const overflow = await page.evaluate((ids) => {
      ...
      return ids.filter((id) => {
        ...
        return r.bottom > H + 1 || r.top < -1 || r.right > W + 1 || r.left < -1;
      });
    }, ids);
