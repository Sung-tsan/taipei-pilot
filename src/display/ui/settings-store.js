...
/** 預設：新玩家走安全；❤️ 上限 3（僅 gentle 用） */
export const DEFAULTS = { mode: /** @type {'safe'} */ ('safe'), heartsMax: 3 };
...
/** @param {StorageLike} [storage] @returns {{ mode:ConsequenceMode, heartsMax:number }} */
export function loadSettings(storage = localStorage) {
  ...
  return { mode: validMode, heartsMax };
}

/** @param {StorageLike} storage @param {{ mode:ConsequenceMode, heartsMax:number }} s */
export function saveSettings(storage, { mode, heartsMax }) {
