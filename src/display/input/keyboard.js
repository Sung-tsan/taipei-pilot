...
// 空戰：F 發射（按住）、Q 換武器（display 端偵測上升緣）。
...
export class KeyboardInput {
  ...
  /**
   * @param {number} dt
   * @returns {{ r:number, p:number, th:number, gearUp:boolean, fire:boolean, weaponSwitch:boolean }}
   */
  read(dt) {
    const d = this._down;
    ...
    return { r, p, th: this.th, gearUp: this.gearUp, fire: d.has('KeyF'), weaponSwitch: d.has('KeyQ') };
  }
}

const KEYS = new Set(['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'KeyW', 'KeyS', 'Space', 'KeyX', 'KeyG', 'KeyF', 'KeyQ']);
