...
## 當前 baseline（最後更新 2026-06-14，含 V2[已 tag v2.0] + V3 v3.0-1~2（天氣+側風亂流受損%））

| 檢查 | 指令 | 期望 |
|------|------|------|
| 型別 | `npm run typecheck` | 0 errors |
| 單元 | `npm test`（vitest） | **306 passed**（26 files；+weapons/enemy-ai/race/balloon/dogfight/minimap/missile/plane-collision/plane-specs/context-keys） |
| 整合 | `npm run e2e`（playwright） | **22 passed**（connect ×11 + flight ×11；含 overlap×3 + RWD×2 + 空戰/F-16/PvP/1v1/競速 regression） |
