...
## 當前 baseline（最後更新 2026-06-13，含 v1.1-0 + v1.1-1）

| 檢查 | 指令 | 期望 |
|------|------|------|
| 型別 | `npm run typecheck` | 0 errors |
| 單元 | `npm test`（vitest） | **88 passed**（9 files；+consequence、+settings-store） |
| 整合 | `npm run e2e`（playwright） | **9 passed**（connect ×7 + flight ×2） |
